...
# `trimesh` is used to estimate volume and inertial properties from meshes
# of links
...
def main():
    ...
    # You can also use your own estimate of total mass if you managed
    # to weight fingrip yourself :)
    total_mass = 18.0
    if len(sys.argv) > 1:
        if float(sys.argv[1]) > 0.0:
            ...
            print(
                "Error: Total mass of fingrip (first argument) must be \
                   positive."
            )
            exit(1)
    print(
        "Estimating inertial properties for each link to add up \
              to %f kg"
        % total_mass
    )

    # Percentage of mass to redistribute from hand to fingers due to internal
    #  mechanical coupling
    ...
    if len(sys.argv) > 2:
        if float(sys.argv[2]) >= 0.0 and float(sys.argv[2]) < 1.0:
            ...
            print(
                "Error: Percentage of hand's mass to redistribute (second \
    ...
    # Compute the total volume of the robot in order to estimate
    ...
    # Redistribute X% of the hand's mass to the fingers due to internal
    # mechanical coupling
    # This improves reliability of grasps and makes fingers less susceptible
    # to disturbances
    ...
    # TODO: Read translation directly from SDF
    #  (currently copied and hard-coded)
    ...
    # Reduce mass and inertia of hand to (1.0-X)% in order to account for
    #  redistribution of its mass
